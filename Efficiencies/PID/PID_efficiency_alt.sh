...
if [[ $SPLIT == "TRUE" ]]; then
    ...
    OUTFILE=$DATA_ROOT/PIDCalib/Results/${YEAR}_$MAG_SHORT/${MODE}_${PARTICLE}_withPIDeffs_alt.root
    LOGFILE=$EFFDIR/logs/${YEAR}_$MAG_SHORT/${MODE}_${PARTICLE}_alt.txt
else 
    INFILE=$DATA_ROOT/MC/$BOD/$MODE/${YEAR}_$MAG_SHORT/${SEL_MODE}_selected.root
    OUTFILE=$DATA_ROOT/PIDCalib/Results/${YEAR}_$MAG_SHORT/${MODE}_withPIDeffs_alt.root
    LOGFILE=$EFFDIR/logs/${YEAR}_$MAG_SHORT/${MODE}_alt.txt
fi
...
# Run command
cd $DATA_ROOT/PIDCalib/PerfHists/Alternative
