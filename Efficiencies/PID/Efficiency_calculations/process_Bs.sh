...
process_mode() {
    ...
    OUTFILE="../Results/Bs${PARTICLE}.param"
    if [[ -f $OUTFILE ]]; then
        rm $OUTFILE
    fi
    ...
    for YEAR in 2011 2012 2015 2016; do
        for MAG in up down; do
            ...
            PID_FILE="/data/lhcb/users/pullen/B02DKstar/PIDCalib/Results/${YEAR}_${MAG}/Bs${PARTICLE}_withPIDeffs.root"
            if [[ -f $PID_FILE ]]; then
