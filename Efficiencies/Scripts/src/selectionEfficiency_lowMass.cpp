...
int main(int argc, char * arg[]) {
    ...
    while(!bk_file.eof()) {
        ...
        bk_file >> bk_mag;
        bk_file >> bk_events;
        orig_events[bk_particle][bk_helicity][bk_mag] = std::stof(bk_events);
        bk_file >> bk_particle;
    }
    bk_file.close();
        ...
        for (auto hel : helicities) {
            ...
            for (auto mag : mags) {
                ...
                TFile * file = TFile::Open((mc_path + particle + "/" + hel +
                            "/2016_" + mag + "/Kpi_selected.root").c_str(), "READ");
                TTree * tree = (TTree*)file->Get("DecayTree");
            double nEntries = (double)tree->GetEntries();
            file->Close();

            // Get efficiency
            double orig = orig_events[particle][hel][mag];
            ...
            // Write to file
            outfile << particle << " " << hel << " " << " " << mag << " "
                << eff << " " << error << std::endl;
            nfile << particle << " " << hel << " " << " " << mag << " "
                << nEntries << std::endl;
