...
int main(int argc, char * argv[]) {
    ...
    std::string mode = "Kpi";
        ...
        for (auto year : years) {
            if (input_year.find(year) != std::string::npos) {
                std::string filepath = path + year + "_down/" + mode + "_withVars_withCuts.root";
                treeMap[mode]->Add(filepath.c_str());
            }
        }

        // Turn on relevant branches only
        treeMap[mode]->SetBranchStatus("*", 0);
        treeMap[mode]->SetBranchStatus("Bd_ConsD_MD", 1);
    ...
    pm.addParam("signal_mean", pr["Kpi_mean"], pr["Kpi_mean"] - 10, pr["Kpi_mean"] + 10);
    ...
    // Bs (crystal ball) shape parameter/*s*/
    ...
    sm.addCrystalBall("signal_CB_L", &Bd_M, &pm, "signal_mean", "signal_sigma_L",
            "signal_alpha_L", "signal_n_L");
    sm.addCrystalBall("signal_CB_R", &Bd_M, &pm, "signal_mean", "signal_sigma_R",
            "signal_alpha_R", "signal_n_R");
    //sm.addCrystalBall("Bs_CB_L", &Bd_M, &pm, "Bs_mean", "Bs_sigma_L",
            //"Bs_alpha_L", "Bs_n_L");
    //sm.addCrystalBall("Bs_CB_R", &Bd_M, &pm, "Bs_mean", "Bs_sigma_R",
            //"Bs_alpha_R", "Bs_n_R");
    ...
    pm.addParam("slope_Kpi", -0.005, -0.5, 0);
    //pm.addParam("slope_KK", -0.005, -0.5, 0);
    //pm.addParam("slope_pipi", -0.005, -0.5, 0);

    // Make combinatorial shapes
    sm.addExponential("expo_Kpi", &Bd_M, &pm, "slope_Kpi");
    //sm.addExponential("expo_KK", &Bd_M, &pm, "slope_KK");
    //sm.addExponential("expo_pipi", &Bd_M, &pm, "slope_pipi");
    ...
    //// B0 low mass shapes
    //sm.addHill("gamma_010", &Bd_M, &pm, "a_gamma", "b_gamma", "csi_gamma_010",
            //"shift", "sigma_gamma_010", "low_ratio", "low_frac");
    //sm.addLittleHorns("gamma_101", &Bd_M, &pm, "a_gamma", "b_gamma", "csi_gamma_101",
            //"shift", "sigma_gamma_101", "low_ratio", "low_frac", "shiftg");
    //sm.addHorns("pi_010", &Bd_M, &pm, "a_pi", "b_pi", "csi_pi_010", "shift", 
            //"sigma_pi_010", "low_ratio", "low_frac");
    //sm.addHill("pi_101", &Bd_M, &pm, "a_pi", "b_pi", "csi_pi_101", "shift", 
            //"sigma_pi_101", "low_ratio", "low_frac");

    //// Bs low mass shapes
    //sm.addHill("Bs_gamma_010", &Bd_M, &pm, "Bs_a_gamma", "Bs_b_gamma", 
            //"Bs_csi_gamma_010", "shift", "Bs_sigma_gamma_010", "low_ratio", 
            //"low_frac");
    //sm.addLittleHorns("Bs_gamma_101", &Bd_M, &pm, "Bs_a_gamma", "Bs_b_gamma", 
            //"Bs_csi_gamma_101", "shift", "Bs_sigma_gamma_101", "low_ratio", 
            //"low_frac", "shiftg");
    //sm.addHorns("Bs_pi_010", &Bd_M, &pm, "Bs_a_pi", "Bs_b_pi", "Bs_csi_pi_010", 
            //"shift", "Bs_sigma_pi_010", "low_ratio", "low_frac");
    //sm.addHill("Bs_pi_101", &Bd_M, &pm, "Bs_a_pi", "Bs_b_pi", "Bs_csi_pi_101", 
    ...
    yields["Kpi"]->addParam("signal", 100, 0, 20000); 
    ...
    //// Total Bs yields
    //yields["piK"]->addParam("Bs", 100, 0, 20000);
    ...
    //yields["Kpi"]->addParam("low", 100, 0, 20000);
    ...
    //yields["piK"]->addParam("low_Bs", 100, 0, 20000);
    ...
    yields["Kpi"]->addParam("expo", 100, 0, 10000);
    //yields["piK"]->addParam("expo", 100, 0, 10000);
    //yields["KK"]->addParam("expo", 100, 0, 10000);
    //yields["pipi"]->addParam("expo", 100, 0, 10000);
    ...
    //std::string mode = "Kpi";
        ...
        //yields_minus.emplace("signal", "signal_minus");
    ...
    RooDataSet combData("combData", "", Bd_M, RooFit::Index(category), RooFit::Import(dataMap));
    ...
    if (sum == "N") {
        ...
        //for (auto mode : modes) {
            simPdf->addPdf(*(fits[mode + "_both"]), (mode + "_both").c_str());
        //}
    }
    ...
    RooFitResult * r = simPdf->fitTo(combData, RooFit::Save(), RooFit::Extended(true),
    ...
    if (sum == "Y") {
        ...
        dataMap["Kpi"]->plotOn(frame);
        fits["Kpi_both"]->plotOn(frame);
        TCanvas * canvas = new TCanvas("Kpi", "", 500, 400);
        frame->Draw();
        canvas->SaveAs("../Plots/Kpi.pdf");
    } 
