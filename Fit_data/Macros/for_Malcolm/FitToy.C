...
// =========================================================================
void FitToy(std::string run_many_string = "N", TString number = "0") {
    ...
    double init_value_n_signal_Kpi = 774.9;
    double final_value_n_signal_Kpi;
    double error_n_signal_Kpi;
    double null_value_n_signal_Kpi;
    double null_error_n_signal_Kpi;
    double init_value_n_expo = 922.6;
    ...
    double pull_init_vs_final_n_signal_Kpi;
    double pull_init_vs_final_n_expo;
    double pull_init_vs_final_R_piK_vs_Kpi;
    double pull_null_n_signal_Kpi;
    double pull_null_n_expo;
    double pull_null_R_piK_vs_Kpi;
    toy_tree->Branch("pull_init_vs_final_R_piK_vs_Kpi", &pull_init_vs_final_R_piK_vs_Kpi);
    toy_tree->Branch("pull_null_R_piK_vs_Kpi", &pull_null_R_piK_vs_Kpi);
    if (free_Kpi_yield) {
        toy_tree->Branch("pull_init_vs_final_n_signal_Kpi", &pull_init_vs_final_n_signal_Kpi);
        toy_tree->Branch("pull_null_n_signal_Kpi", &pull_null_n_signal_Kpi);
    }
    if (free_expo_yield) {
        toy_tree->Branch("pull_init_vs_final_n_expo", &pull_init_vs_final_n_expo);
        toy_tree->Branch("pull_null_n_expo", &pull_null_n_expo);
    }

    // Make branches for significance and status
    double significance;
    int status;
    toy_tree->Branch("significance", &significance);
    ...
    for (int i = 0; i < n_toys; i++) {
        ...
        RooRealVar * toy_n_signal_Kpi = new RooRealVar("toy_n_signal_Kpi", "", 
                init_value_n_signal_Kpi);
        RooRealVar * toy_n_expo_piK = new RooRealVar("toy_n_expo_piK", "", 
                init_value_n_expo);
        RooRealVar * toy_R_piK_vs_Kpi = new RooRealVar("toy_R_piK_vs_Kpi", "", 
                init_value_R_piK_vs_Kpi);
        ...
        // ======================
        significance = sqrt(2 * (NLL_null - NLL));
        ...
        if (run_many) { 
            ...
            // Fill values of parameters
            final_value_n_signal_Kpi = n_signal_Kpi->getVal();
            final_value_n_expo = n_expo_piK->getVal();
            final_value_R_piK_vs_Kpi = R_piK_vs_Kpi->getVal();
            null_value_n_signal_Kpi = null_n_signal_Kpi->getVal();
            null_value_n_expo = null_n_expo_piK->getVal();
            error_n_signal_Kpi = n_signal_Kpi->getError();
            error_n_expo = n_expo_piK->getError();
            error_R_piK_vs_Kpi = R_piK_vs_Kpi->getError();
            null_error_n_signal_Kpi = null_n_signal_Kpi->getError();
            null_error_n_expo = null_n_expo_piK->getError();

            // Calculate pulls
            // Initial value vs. final value in normal fit
            pull_init_vs_final_n_signal_Kpi = (final_value_n_signal_Kpi - init_value_n_signal_Kpi) / error_n_signal_Kpi;
            pull_init_vs_final_n_expo = (final_value_n_expo - init_value_n_expo) / error_n_expo;
            pull_init_vs_final_R_piK_vs_Kpi = (final_value_R_piK_vs_Kpi - init_value_R_piK_vs_Kpi) / error_R_piK_vs_Kpi;

            // Normal value vs. null hypotheses value
            pull_null_n_signal_Kpi = (null_value_n_signal_Kpi - final_value_n_signal_Kpi) / 
                sqrt(pow(error_n_signal_Kpi, 2) + pow(null_error_n_signal_Kpi, 2));
            pull_null_n_expo = (null_value_n_expo - final_value_n_expo) / 
                sqrt(pow(error_n_expo, 2) + pow(null_error_n_expo, 2));
            pull_null_R_piK_vs_Kpi = (null_value_R_piK_vs_Kpi - final_value_R_piK_vs_Kpi) / error_R_piK_vs_Kpi;

            // Get status: combine status of both fits
            status = result->status() + result_null->status();
